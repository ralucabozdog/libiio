...
zephyr_create_context(const struct iio_context_params *params, const char *args)
{
	...
	STRUCT_SECTION_FOREACH(iio_device_info, iio_device_info) {
		...
		api = DEVICE_API_GET(iio_device, dev);
			...
			size_t nb_samples, bool enable, bool cyclic)
{
	...
	const struct device *trig_dev = (const struct device *) iio_device_get_pdata(trig);
	const struct iio_device_trigger_config *trig_config = (const struct iio_device_trigger_config *)trig_dev->config;
	...
	if (enable) {
		...
		return trig_config->ops->subscribe(&pdata->node);
	}
