		...
		struct iio_device *iio_device)
{
	...
	for (index = 0; index < config->num_channels; index++) {
		...
		const struct iio_data_format fmt = {
			.length = (channel->resolution / 8 + (channel->resolution % 8 != 0)) * 8,
			.bits = channel->resolution,
			.is_signed = is_signed,
			.with_scale = true,
			.scale = ((double)channel->vref_mv) / (double)(1u << scale_resolution),
			.is_be = true,
		};
...
static const char *iio_device_adc_get_buffer_name(const struct device *dev)
{
	const struct iio_device_adc_config *config = dev->config;
...
static DEVICE_API(iio_device, iio_device_adc_driver_api) = {
	...
	.get_buffer_name = iio_device_adc_get_buffer_name,
};
