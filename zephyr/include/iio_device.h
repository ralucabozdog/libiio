		...
		char *dst, size_t len)
{
	...
	if (iio_device_is_trigger(iio_device)) {
		return iio_device_trigger_read_attr(iio_device, attr, dst, len);
	}
		...
		const char *src, size_t len)
{
	...
	if (iio_device_is_trigger(iio_device)) {
		return iio_device_trigger_write_attr(iio_device, attr, src, len);
	}
